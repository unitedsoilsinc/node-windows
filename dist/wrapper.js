...
const { errorMonitor } = __webpack_require__(1);

// Handle input parameters
var Logger = __webpack_require__(2),
  optimist = __webpack_require__(6),
  ...
  fork = (__webpack_require__(5).fork),
...
if (argv.d) {
  if (!(__webpack_require__(10).existsSync)(p.resolve(argv.d))) {
 ...
 *     });
 */
var wincmd = __webpack_require__(3),
  exec = (__webpack_require__(5).exec),
...
var params = function (binPath, options, callback) {
  ...
  return { binPath: binPath, options: options, callback: callback };
}
...
module.exports = {
   ...
   */
  elevate: function (cmd, binPath, options, callback) {
    var p = params(binPath, options, callback);
    exec('"' + path.join(p.binPath || bin, 'elevate', 'elevate.cmd') + '" ' + cmd, p.options, p.callback);
  },
   ...
   * The callback function passed to `require('child_process').exec(cmd,options,<CALLBACK>)`.
   */
  sudo: function (cmd, password, binPath, options, callback) {
    password = password || '';
    ...
    var p = params(binPath, options, callback);
    exec(path.join(p.binPath || bin, 'sudowin', 'sudo.exe') + ' ' + (password !== '' ? '-p ' + password : '') + cmd, p.options, p.callback);
  }
}
...
const {Yargs, processArgv} = __webpack_require__(7);
...
function Argv(processArgs, cwd) {
  const argv = Yargs(processArgs, cwd, __webpack_require__(30));
  singletonify(argv);
...
const parser = new YargsParser({
    ...
    require: (path) => {
        if (true) {
            return __webpack_require__(14)(path);
        }
        else {}
    }
});
...
const wrap = __webpack_require__(21);
function ui(opts) {
    return cliui(opts, {
        stringWidth,
        stripAnsi,
...
const stripAnsi = __webpack_require__(17);
const isFullwidthCodePoint = __webpack_require__(19);
const emojiRegex = __webpack_require__(20);
...
const cssKeywords = __webpack_require__(25);
...
const { dirname, resolve } = __webpack_require__(4);
const { readdirSync, statSync } = __webpack_require__(10);
