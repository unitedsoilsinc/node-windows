  ...
  Object.defineProperties(this, {
    ...
    _xml: {
      ...
      get: function () {
        ...
        let wrapper = this.libPath
          ? path.resolve(path.join(this.libPath,'wrapper.js'))
          : path.resolve(path.join(__dirname, 'wrapper.js'));
    ...
    */
    ...
    /**
     * @cfg {String} binPath
     * The absolute path to the bin directory containing winsw.exe, etc.  This is useful if
     * bundling your app (e.g. using Electron).
     * If omitted __dirname/../bin is used.
     */
    binPath: {
      enumerable: true,
      writable: true,
      configurable: false,
      value: config.binPath !== undefined ? require('path').resolve(config.binPath) : undefined
    },

    /**
     * @cfg {String} libPath
     * The absolute path to the lib directory containing wrapper.js, etc.  This is useful if
     * bundling your app (e.g. using Electron).
     * If omitted __dirname is used.
     */
    libPath: {
      enumerable: true,
      writable: true,
      configurable: false,
      value: config.libPath !== undefined ? require('path').resolve(config.libPath) : undefined
    },
      ...
      value: function (dir) {
        ...
        fs.exists(dir, function (exists) {
          ...
          fs.writeFile(path.resolve(dir, me.id + '.xml'), me._xml, function () {
            // Write the exe file
            winsw.createExe(me.id, dir, this.binPath, function () {
    ...
    execute: {
      ...
      value: function (cmd, options, callback) {
        ...
        wincmd.isAdminUser(this.binPath, function (isAdmin) {
          if (isAdmin) {
            ...
            if (me.user.account !== null && me.user.password !== null) {
              ...
              wincmd.sudo(cmd, me.sudo.password || '', me.binPath, options, callback);
            } else {
              wincmd.elevate(cmd, me.binPath, options, callback)
            }
          } else {
